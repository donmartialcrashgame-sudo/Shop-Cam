...
function mountSidebar(active='dashboard'){
  ...
  if(!side){
    ...
    if(main) main.classList.add('sc-with-sidebar');

    const out=document.getElementById('sc-signout');
  ...
  if(!menu){
    ...
    menu.id='sc-mobile-menu';
    menu.className='sc-mobile-menu';
    menu.type='button';
...
function toast(message,type='info'){
  ...
  if(!box){
    ...
    box.id='sc-toast';
    box.style.cssText='position:fixed;left:50%;bottom:25px;transform:translateX(-50%);z-index:10000;padding:12px 17px;border-radius:10px;background:#171717;color:#fff;font:700 11px Arial;box-shadow:0 8px 30px #0003;max-width:90vw;text-align:center';
    document.body.appendChild(box);
  }
  box.textContent=message;
  box.style.background=type==='success'?'#16803c':type==='error'?'#c71920':'#171717';
  box.style.display='block';
  clearTimeout(box._timer);
  box._timer=setTimeout(()=>box.style.display='none',2400);
}
...
const customerPages={
  ...
  '/faq.html':'faq',
  '/customer-care.html':'care',
  '/report.html':'report',
  '/terms.html':'terms',
  '/about.html':'about',
  '/view-details.html':'dashboard',
  '/checkout.html':'cart',
  '/receipt.html':'orders'
};
...
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded',autoMountSidebar,{once:true});
}else{
  autoMountSidebar();
}
