...
function mountSidebar(active='dashboard'){
 ...
 if(!menu){
  menu=document.createElement('button');menu.id='sc-mobile-menu';menu.className='sc-mobile-menu';menu.type='button';menu.setAttribute('aria-label','Open menu');menu.setAttribute('aria-expanded','false');menu.textContent='☰';document.body.appendChild(menu);
 }
 const overlay=document.getElementById('sc-overlay');
 const close=()=>{side.classList.remove('open');menu.classList.remove('open');menu.textContent='☰';menu.setAttribute('aria-expanded','false');overlay?.classList.remove('show');document.body.classList.remove('sc-sidebar-open')};
 menu.onclick=()=>{const open=!side.classList.contains('open');side.classList.toggle('open',open);menu.classList.toggle('open',open);menu.textContent=open?'×':'☰';menu.setAttribute('aria-expanded',String(open));overlay?.classList.toggle('show',open);document.body.classList.toggle('sc-sidebar-open',open)};
...
function autoMountSidebar(){const path=location.pathname.replace(/\\+/g,'/');const key=customerPages[path]||customerPages[path.replace(/\\/$/,'')];if(!key)return;mountSidebar(key)}
